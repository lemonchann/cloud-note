...
- poetry
- conda
-  miniconda 最香，不仅管理python包方便，还可以安装gcc，大多数c++库，与系统完全隔离。 

virtualenv在升级到20.0以后和venv的体验已经差不多了，但它们都只提供环境隔离的功能，缺少依赖版本解析的能力。pip 最近发布的20.1beta版本里面已经加入了一个依赖解析器(resolver)所以，这两位要加上pip才是完全体

Pipenv 和Poetry差不多，可以理解为Pip + resolver + virtualenv，其中Poetry界面，易用性上更胜一筹，Pipenv先发优势所以更加流行一些，但是被Poetry狂追。

Conda（conda才是包管理器，miniconda和anaconda是内置了conda的Python发行版）作为环境隔离和依赖安装的一体化解决方案，也有很方便的功能，Anaconda的包仓库更是对科学计算的技术栈非常友好，但不推荐做web开发的同学使用。
...
http://redisdoc.com/topic/index.html    命令参考

http://try.redis.io/  在线redis命令终端

#### 命令

##### EXPIRE

可以对一个已经带有生存时间的 `key` 执行 [EXPIRE](http://redisdoc.com/key/expire.html#expire) 命令，新指定的生存时间会取代旧的生存时间。

##### INCR

 如果 key 不存在，那么 key 的值会先被初始化为 0 ，然后再执行 INCR 操作。  执行 INCR 命令之后 key 的值。 

##### eval 执行lua脚本

 [redis的Lua脚本控制（原子性）](https://segmentfault.com/a/1190000019676878)
...
#### linux命令大全  https://man.linuxde.net/yum 
...
#### makefile语法学习
   ...
   -rm -f *.o $(TARGET) $(LIBTARGET)  // -rm 前面的-表示忽略rm执行中的错误

   $(patsubst %.cpp,%.o,$(wildcard *.cpp))  //把当前目录下所有的.cpp的文件替换.cpp成.o 
   ...
   原文链接：https://blog.csdn.net/zqj6893/article/details/9066225
...
#### notepad++在行未加标点
 ...
 https://blog.csdn.net/u011598529/article/details/47833717 

#### [win查看MD5 SHA](https://blog.csdn.net/p358278505/article/details/70672169)

#### yaml

#### shell终端记录网站  https://asciinema.org/ 
...
终端复制到编辑器：选中->点一遍右键，**再点一遍右键 才能正常复制黏贴**

##### [vscode插件推荐](https://blog.fundebug.com/2018/07/24/vs-extensions/)

##### [fileheader-vscode插件](https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE)

自动生成头部注释 koroFileHeader\

ctrl+alt+i -文件头注释  ctrl+alt+T-函数名注释

 [vscode添加头文件#ifdef宏定义]( https://blog.csdn.net/weixin_45461426/article/details/105936955 ) 

##### vscode快捷键

 https://nshen.net/article/2015-11-20/vscode/ 

- 主命令框 F1 或ctrl+shift +p   按一下`Backspace`会进入到`Ctrl+P`模式里 

- 查找所有引用了选中symbol的代码 `Shift + F12`
- 跳转到symbol的定义处 `F12`
- 瞥见下symbol的定义 `Alt + F12`
- ctrl+p 搜索文件 
- ctrl+r 搜索近期文件
- ctrl+shift+o 搜索文件内符号和函数
- 按住ctrl 光标放上函数能看到定义

##### vscode自适应编码格式utf8  和gb

files.autoGuessEncoding

##### vscode格式化代码

Shift+Alt+F  在配置搜索clang_ormate fallback style更改
...
### SRE( Site reliability engineering ) 和DevOps

 https://juejin.im/post/5b95448cf265da0aa41e4e6c 
